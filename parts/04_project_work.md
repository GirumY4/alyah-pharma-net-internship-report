...
for pharmaceutical logistics in Ethiopia [1]. The platform was built within
...
cloud platforms: the shared backend on Render [22], the SaaS dashboard on Vercel [23],
and the database on MongoDB Atlas [16]. It implements multi-tenant data isolation
...
collections (simulating ALCOA+ and 21 CFR Part 11 data integrity) [10], [11], FEFO
batch selection on order approval (aligning with the EFDA Traceability
Directive) [12], and a public marketplace aggregation pipeline that joins
...
29148-aligned SRS [1], [9], an SDS [2], an API contract [3], and a database schema reference
[4] — treated as the single source of truth throughout implementation.
...
pharmacies in Ethiopia [1], [2]. Three interrelated gaps define the current state.
...
consequence is expired medicine reaching patients [1].
...
attributable records of every stock mutation [12], [13]. Most independent pharmacies
...
- **SO-2.** Implement a complete medicine catalogue module with batch-level
  ...
  FR-2.1 through FR-2.6 of the SRS [1].
- **SO-3.** Implement an immutable GRN/GIN stock ledger in which every
  ...
  entry, satisfying FR-2.7 and NFR-4.4 [1].
- **SO-4.** Implement a cross-tenant consumer order lifecycle in which a
  ...
  selection inside a database transaction, satisfying FR-3.1 through FR-3.5 [1].
- **SO-5.** Implement a payment recording module that aggregates completed
  ...
  satisfying FR-4.1 through FR-4.4 [1].
- **SO-6.** Implement tenant-scoped and platform-wide reporting endpoints
  ...
  tenant platform metrics, satisfying FR-5.1 through FR-5.5 [1].
...
- **SO-8.** Deploy the platform to production on three cloud services
  ...
  management, and a post-deployment verification checklist [1].
...
Life cycle processes — Requirements engineering) [9]. Rather than treating
specifications as an up-front gate and then switching to implementation,
the four governing documents — the SRS [1], the SDS [2], the API documentation [3],
and the database schema reference [4] — were maintained as living artifacts
...
done until its mapped test case had been exercised manually [1].
...
collections [4], the schema-level constraints and indexes for each collection,
...
frontend, which the implementation then followed exactly [2].
...
mapping to SRS requirements [3]. A companion database schema reference
documented every collection, field, index, and virtual property [4]. Together
these two documents allowed the frontend and mobile teams to build against
stable contracts while the backend was still being implemented [5].

### Iterative Phased Implementation

Implementation proceeded in four two-week phases aligned with the SDS [2]:

<!-- table: Phased implementation plan aligned with the Software Design Specification [2] -->
...
and short-lived feature branches [5]. Commits followed the Conventional Commits
...
- **Manual API exercise** using Postman and curl for every endpoint against
  the requirement traceability matrix [25];
...
- **Tenant isolation tests** in which a pharmacy manager's JWT was used to
  attempt cross-tenant access and verified to fail on every scoped endpoint [1];
- **Concurrency tests** on the atomic stock deduction path to verify no
  negative stock could be produced under simulated simultaneous approvals [2];
- **Deployment verification** via a post-deployment checklist executed
  against the live production environment on Render, Vercel, and Atlas [22], [23].
...
The delivered platform implements the complete surface defined in the SRS [1].
The mapping between the seven specific objectives and the implemented
artefacts is summarized below.

<!-- table: Traceability of specific objectives to implemented modules [8] -->
...
has a corresponding implementation mechanism [1], [3].

### Deployment Results

The platform was deployed to production across three services:

<!-- table: Production deployment configuration and verification status [22], [23] -->
...
1. **Code-level enforcement.** Every tenant-scoped query is filtered by a
   ...
   body or query string are ignored for non-admin roles [14].
...
`findOneAndReplace`, `deleteOne`, `deleteMany`, and `bulkWrite` operation [4].
...
original record of stock history — the four properties required by ALCOA+ [11].
...
- **Using atomic database operations for stock mutation.** The `$inc`
  ...
  otherwise have surfaced only under production concurrency [16].
...
- The **React SaaS Dashboard**, built with React 19, Vite, TypeScript, and
  MUI [18], [20], [21], is the primary workspace for Pharmacy Managers and System
...
middleware pipeline consisting of: Helmet security headers [27], CORS with an
...
`req.user`) [14], role-based access control middleware (which gates endpoints by
...
details? } }` envelope documented in the API contract [3]. A tenant gate
...
`totalStock` [16]. This operation either decrements stock by exactly the
...
the earliest-expiring batches first (FEFO) [12], with one immutable GIN ledger
...
function, which is the reason the audit trail is complete [10], [11].
...
platform available to anonymous visitors [3].
...
user-friendly messages through a central `errorMapper` utility [18].
...
5. **Integrate EFDA GS1 reporting** [12], [13] so that the GTIN and batch data
